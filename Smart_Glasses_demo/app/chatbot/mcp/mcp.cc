...
namespace app
{
    namespace chatbot
    {
        namespace mcp
        {
            ...
            namespace
            {
                constexpr double SUCCESS_RATE_WARNING_THRESHOLD = 95.0; // 工具调用成功率告警阈值
            }
            ...
            public:
                ...
                std::string handleToolsCall(int id, const json& params)
                {
                    ...
                    try
                    {
                        ...
                        {
                            LOG_ERROR("MCP", "%s", parse_error->c_str());
                        ...
                        {
                            LOG_ERROR("MCP", "%s", populate_error->c_str());
                            stats.tools_call_errors.fetch_add(1, std::memory_order_relaxed);
