    ...
    namespace chatbot
    {
        namespace activation
        {
            ...
            struct ActivationResult
            {
                ...
                ActivationResult()
                    : status(ActivationStatus::UNKNOWN), error(ActivationError::NONE),
                      http_status_code(0), check_timestamp(0)
                {
                }
            ...
            struct ActivationConfig
            {
                ...
                // 默认常量
                static constexpr int  DEFAULT_CONNECT_TIMEOUT_MS = 5000;
                ...
                static constexpr int  DEFAULT_POLL_INTERVAL_SEC  = 5;
                static constexpr int  DEFAULT_POLL_TIMEOUT_SEC   = 300;
                static constexpr bool DEFAULT_ENABLE_PROGRESS    = true;
                static constexpr bool DEFAULT_VERIFY_SSL         = false;
                static constexpr bool DEFAULT_ENABLE_AUTO_RETRY  = true;
                static constexpr bool DEFAULT_ENABLE_LOGGING     = true;
                ...
                // HTTP配置
                int connect_timeout_ms = DEFAULT_CONNECT_TIMEOUT_MS; // 连接超时
                int request_timeout_ms = DEFAULT_REQUEST_TIMEOUT_MS; // 请求超时
                int max_retries        = DEFAULT_MAX_RETRIES;        // 最大重试次数
                int retry_delay_ms     = DEFAULT_RETRY_DELAY_MS;     // 重试间隔

                // 轮询配置
                int  poll_interval_sec = DEFAULT_POLL_INTERVAL_SEC; // 轮询间隔
                int  poll_timeout_sec  = DEFAULT_POLL_TIMEOUT_SEC;  // 轮询超时（5分钟）
                bool enable_progress_callback = DEFAULT_ENABLE_PROGRESS; // 启用进度回调

                // SSL配置
                bool verify_ssl = DEFAULT_VERIFY_SSL; // 是否验证SSL证书（默认关闭，与V1一致）

                // 功能开关
                bool enable_auto_retry       = DEFAULT_ENABLE_AUTO_RETRY; // 启用自动重试
                bool enable_detailed_logging = DEFAULT_ENABLE_LOGGING;    // 启用详细日志
            };
             ...
             * @brief 激活状态变化回调
             */
            using ActivationStatusCallback =
             ...
             * @brief 激活错误回调
             */
            using ActivationErrorCallback =
                std::function<void(ActivationError error, const std::string& message)>;
            ...
            public:
                 ...
                 */
                bool startPolling(const std::string& mac, const std::string& uuid,
                                  int timeout_sec = ActivationConfig::DEFAULT_POLL_TIMEOUT_SEC);
