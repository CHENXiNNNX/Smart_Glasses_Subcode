 ...
 */
 ...
 public:
     ...
     explicit Impl(const ActivationConfig& cfg)
         : config(cfg) {
         LOG_DEBUG("Activation", "Impl创建");
         
         // 创建HTTP客户端
         http_client = std::make_unique<http::HttpClient>();
         
         if (!http_client->isValid()) {
             LOG_ERROR("Activation", "HTTP客户端创建失败");
         }
     }
     
     ~Impl() {
         LOG_DEBUG("Activation", "Impl销毁中...");
         stopPollingThread();
         LOG_DEBUG("Activation", "Impl已销毁");
     }
     ...
     ActivationResult checkActivationInternal(const std::string& mac, const std::string& uuid) {
         ...
         LOG_DEBUG("Activation", "检查激活状态: Device-Id=%s", mac.c_str());
         ...
         for (retry = 0; retry < config.max_retries; retry++) {
             if (retry > 0) {
                 LOG_INFO("Activation", "重试 %d/%d，等待%dms", 
            ...
            LOG_WARN("Activation", "HTTP请求失败: %s", http_resp.error_message.c_str());
        }
        ...
        stats.check_count.fetch_add(1, std::memory_order_relaxed);
         ...
         if (!http_resp.success) {
             ...
             LOG_ERROR("Activation", "激活检查失败，已重试%d次", retry);
             return result;
         }
         ...
         try {
             ...
             if (response.contains("activation") && 
                 ...
                 LOG_INFO("Activation", "设备未激活，激活码: %s",
             ...
             LOG_ERROR("Activation", "响应解析失败: %s", e.what());
             LOG_DEBUG("Activation", "响应内容: %s", http_resp.body.c_str());
         }
     ...
     bool startPollingThread(const std::string& mac, const std::string& uuid, int timeout_sec) {
         ...
         polling_thread = std::make_unique<std::thread>([this, mac, uuid, timeout_sec]() {
            ...
            ActivationResult result = checkActivationInternal(mac, uuid);
             ...
             if (result.hasError()) {
                 LOG_ERROR("Activation", "初次检查失败: %s", result.error_message.c_str());
             ...
             // 未激活，显示激活信息
             LOG_INFO("Activation", "⚠ 设备未激活");
             LOG_INFO("Activation", "  激活URL: %s", config.activation_url.c_str());
             LOG_INFO("Activation", "  激活码: %s", result.activation_code.c_str());
             LOG_INFO("Activation", "  轮询间隔: %d秒", config.poll_interval_sec);
             LOG_INFO("Activation", "========================================");
             ...
             while (elapsed < timeout_sec && !should_stop_polling.load(std::memory_order_acquire)) {
                 ...
                 LOG_DEBUG("Activation", "检查激活中... (%d/%d 秒)", elapsed, timeout_sec);
                 ...
                 if (result.hasError()) {
                     LOG_WARN("Activation", "检查失败，继续轮询... (错误: %s)",
             ...
             if (elapsed >= timeout_sec) {
                 LOG_WARN("Activation", "========================================");
                 LOG_WARN("Activation", "  ✗ 激活超时 (%d 秒)", timeout_sec);
                 LOG_WARN("Activation", "========================================");
     ...
     void invokeStatusCallback(ActivationStatus status, const ActivationResult& result) {
         ...
         if (status_callback) {
             try {
                 ...
                 LOG_ERROR("Activation", "状态回调异常: %s", e.what());
             }
         }
     }
     ...
     void invokeProgressCallback(int elapsed_sec, int total_sec) {
         ...
         if (progress_callback) {
             try {
                 ...
                 LOG_ERROR("Activation", "进度回调异常: %s", e.what());
             }
         }
     }
     ...
     void invokeErrorCallback(ActivationError error, const std::string& message) {
         ...
         if (error_callback) {
             try {
                 ...
                 LOG_ERROR("Activation", "错误回调异常: %s", e.what());
             }
         }
     }
 };
...
DeviceActivation::DeviceActivation(const ActivationConfig& config)
    : pImpl_(std::make_unique<Impl>(config)) {
    http::ensureCurlGlobalInit();
    LOG_INFO("Activation", "设备激活管理器已创建");
}
...
DeviceActivation::~DeviceActivation() {
    ...
    stopPolling();
    logStats();
    LOG_INFO("Activation", "设备激活管理器已销毁");
}
 ...
 std::future<ActivationResult> DeviceActivation::checkActivationAsync(
 ...
 bool DeviceActivation::startPolling(const std::string& mac, 
                                      const std::string& uuid,
                                      int timeout_sec) {
 ...
 ActivationResult DeviceActivation::waitForPollingComplete(int timeout_ms) {
     if (!pImpl_->polling_thread || !pImpl_->polling_thread->joinable()) {
         LOG_WARN("Activation", "没有活动的轮询可等待");
 ...
 bool DeviceActivation::updateConfig(const ActivationConfig& config) {
     if (isPolling()) {
         LOG_WARN("Activation", "轮询运行中，无法更新配置");
         return false;
     }
     
     pImpl_->config = config;
     LOG_INFO("Activation", "配置已更新");
     return true;
 }
 ...
 void DeviceActivation::logStats() const {
     ...
     LOG_INFO("Activation", "=== 设备激活统计 ===");
     LOG_INFO("Activation", "  检查次数:     %llu", total);
     LOG_INFO("Activation", "  成功:         %llu", success);
     LOG_INFO("Activation", "  失败:         %llu", failed);
     LOG_INFO("Activation", "  网络错误:     %llu", network_err);
     LOG_INFO("Activation", "  解析错误:     %llu", parse_err);
     LOG_INFO("Activation", "  重试次数:     %llu", retries);
     
     if (total > 0) {
         uint64_t avg_time = pImpl_->stats.avg_check_time_us.load();
         LOG_INFO("Activation", "  平均耗时:     %llu ms", avg_time / 1000);
         
         double success_rate = (double)success / total * 100.0;
         LOG_INFO("Activation", "  成功率:       %.2f%%", success_rate);
         
         if (success_rate < 90.0) {
             LOG_WARN("Activation", "成功率偏低，请检查网络稳定性");
         }
     }
 }
