...
public:
    ...
    ProtocolError enqueueMessage(const char* buffer, size_t size) {
        ...
        if (message_queue.size() >= config.message_queue_size) {
            ...
            LOG_WARN("ProtocolHandler", "Message queue full (%zu), dropping oldest message", 
    ...
    bool dispatchMessage(MessageType type, const json& j) {
        switch (type) {
            ...
            default:
                LOG_WARN("ProtocolHandler", "Unknown message type: %d", static_cast<int>(type));
                return false;
        }
    }
    ...
    bool handleHelloMessage(const json& j) {
        try {
            ...
            if (j.contains("audio_params") && j["audio_params"].is_object() && !j["audio_params"].is_null()) {
                ...
                if (ap.contains("sample_rate") && ap["sample_rate"].is_number()) {
                    ...
                    if (sr >= 8000 && sr <= 48000) {
                        ...
                        LOG_WARN("ProtocolHandler", "Invalid sample_rate: %d, using default", sr);
                ...
                if (ap.contains("channels") && ap["channels"].is_number()) {
                    ...
                    if (ch >= 1 && ch <= 2) {
                        ...
                        LOG_WARN("ProtocolHandler", "Invalid channels: %d, using default", ch);
                ...
                if (ap.contains("frame_duration") && ap["frame_duration"].is_number()) {
                    ...
                    if (fd >= 10 && fd <= 60) {
                        ...
                        LOG_WARN("ProtocolHandler", "Invalid frame_duration: %d, using default", fd);
                ...
                msg.audio_params.channels = config.default_channels;
                msg.audio_params.frame_duration = config.default_frame_duration;
                LOG_WARN("ProtocolHandler", "audio_params missing or null, using defaults");
            }
            ...
            LOG_ERROR("ProtocolHandler", "Failed to handle Hello message: %s", e.what());
            return false;
        }
    }
        ...
        try {
            ...
            if (j.contains("state") && j["state"].is_string()) {
                ...
                if (state_str == "start") {
                    ...
                    LOG_WARN("ProtocolHandler", "Unknown listen state: %s", state_str.c_str());
                    msg.state = ListenState::START;
                }
            } else {
                msg.state = ListenState::START;
            ...
            LOG_ERROR("ProtocolHandler", "Failed to handle Listen message: %s", e.what());
            return false;
        }
    }
    ...
    bool handleSTTMessage(const json& j) {
        try {
            ...
            LOG_DEBUG("ProtocolHandler", "-> STT: \"%s\" (final: %s)",
    ...
    bool handleTTSMessage(const json& j) {
        try {
            ...
            if (state_str == "start") {
                ...
                LOG_DEBUG("ProtocolHandler", "<- TTS: state=sentence_start, text=\"%s\"", msg.text.c_str());
                invokeTTSCallback(msg);
            } else if (state_str == "stop") {
                TTSMessage msg;
                msg.session_id = j.value("session_id", "");
                ...
                // 未知状态
                LOG_DEBUG("ProtocolHandler", "<- TTS: state=%s (ignored)", state_str.c_str());
            }
    ...
    bool handleMCPMessage(const json& j) {
        try {
            ...
            LOG_DEBUG("ProtocolHandler", "<- MCP: payload size=%zu", mcp_payload.size());
    ...
    bool handleErrorMessage(const json& j) {
        try {
            ...
            LOG_ERROR("ProtocolHandler", "Failed to handle Error message: %s", e.what());
            return false;
        }
    }
    ...
    void invokeHelloCallback(const HelloMessage& msg) {
        ...
        if (hello_callback) {
            try {
                ...
                LOG_ERROR("ProtocolHandler", "Hello callback runtime_error: %s", e.what());
                stats.callback_exceptions.fetch_add(1, std::memory_order_relaxed);
            } catch (const std::logic_error& e) {
                LOG_ERROR("ProtocolHandler", "Hello callback logic_error: %s", e.what());
                stats.callback_exceptions.fetch_add(1, std::memory_order_relaxed);
            } catch (const std::exception& e) {
    ...
    void invokeProtocolError(ProtocolError error, const std::string& detail) {
        ...
        if (protocol_error_callback) {
            try {
                ...
                LOG_ERROR("ProtocolHandler", "Protocol error callback exception: %s", e.what());
            }
        }
    }
...
std::string ProtocolHandler::generateHelloMessage(int sample_rate, 
    ...
    if (sample_rate < 8000 || sample_rate > 48000) {
        LOG_WARN("ProtocolHandler", "Invalid sample_rate: %d, using default", sample_rate);
        sample_rate = pImpl_->config.default_sample_rate;
    }
    ...
    if (frame_duration < 10 || frame_duration > 60) {
        LOG_WARN("ProtocolHandler", "Invalid frame_duration: %d, using default", frame_duration);
        frame_duration = pImpl_->config.default_frame_duration;
    }
...
void ProtocolHandler::logStats() const {
    ...
    if (total > 0) {
        ...
        if (avg_time > 1000) {
            LOG_WARN("ProtocolHandler", "Average parse time > 1ms, consider optimization");
        }
    }
    ...
    if (total > 100) {
        ...
        if (error_rate > 1.0) {
            LOG_WARN("ProtocolHandler", "Error rate: %.2f%% (review message format)", error_rate);
        }
        
        if (exception_rate > 0.1) {
            LOG_WARN("ProtocolHandler", "Exception rate: %.2f%% (review callbacks)", exception_rate);
        }
        
        if (overflows > 0) {
            LOG_WARN("ProtocolHandler", "Queue overflows: %llu (consider increasing queue size)", 
                    overflows);
        }
    }
}
