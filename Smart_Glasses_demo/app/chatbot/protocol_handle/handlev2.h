/**
 * @file handlev2.h
 * @brief xiaozhi AI协议处理模块V2 - 现代C++重写版本
 * @details 特性：
 *          - RAII资源管理
 *          - 智能指针（无裸指针）
 *          - 消息内存池（零拷贝传输）
 ...
 * @brief 协议错误回调
 */
using ProtocolErrorCallback = std::function<void(ProtocolError error, const std::string& detail)>;
...
/**
 * @brief xiaozhi协议处理器V2
 * @details 现代C++重写的协议处理器，特性：
 *          - RAII自动资源管理
 *          - 智能指针，无裸指针
 *          - 消息内存池，零拷贝传输
 *          - 哈希表O(1)查找
...
public:
     ...
     * @param config 协议配置
     */
    explicit ProtocolHandlerV2(const ProtocolConfig& config = ProtocolConfig());
     ...
     * @return JSON字符串
     */
    std::string generateHelloMessage(int sample_rate = -1, 
                                     int channels = -1, 
    ...
    // 禁止拷贝和赋值
    ProtocolHandlerV2(const ProtocolHandlerV2&) = delete;
    ProtocolHandlerV2& operator=(const ProtocolHandlerV2&) = delete;
