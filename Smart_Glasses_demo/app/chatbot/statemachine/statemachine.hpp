    ...
    namespace chatbot
    {
        namespace statemachine
        {
            ...
            struct StateTransition
            {
                ...
                StateTransition() : from_state(AIState::IDLE), to_state(AIState::IDLE), timestamp(0)
                {
                }

                StateTransition(AIState from, AIState to, uint64_t ts, const std::string& event,
                                const std::string& info = "")
                    : from_state(from), to_state(to), timestamp(ts), trigger_event(event),
                      detail(info)
                {
                }
            };
             ...
             */
            using ErrorCallback =
                std::function<void(StateMachineError error, const std::string& message)>;
