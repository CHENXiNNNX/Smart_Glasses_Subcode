 ...
 */
 ...
 // 命名空间别名
 namespace protocol_ns = app::chatbot::protocol_handle;
 namespace statemachine_ns = app::chatbot::statemachine;
 namespace mcp_ns = app::chatbot::mcp;
 namespace activation_ns = app::chatbot::activation;
 namespace wakeword_ns = app::chatbot::wakeword;
 namespace websocket_ns = app::protocol::websocket;
 namespace udp_ns = app::protocol::udp;
 namespace audio_ns = app::media::audio;
 ...
 public:
    ...
    // 重连管理
    std::atomic<uint64_t> last_reconnect_check{0};
    static constexpr uint64_t RECONNECT_CHECK_INTERVAL_US = 5000000; // 5秒
    ...
    mutable std::mutex mutex;
     ...
     ChatbotError initializeDeviceInfo() {
         ...
         if (config.device_id.empty()) {
             device_id = tool::mac::getWirelessMacAddress();
             if (device_id.empty()) {
         ...
         if (config.client_id.empty()) {
             client_id = tool::uuid::generateUUID(config.config_file_path);
             if (client_id.empty()) {
     ...
     ChatbotError initializeCoreModules() {
         ...
         try {
             ...
             if (config.enable_mcp_tools) {
                 int tool_count = app::tool::mcp_tool::McpToolManagerV2::register_all_tools(*mcp_server);
     ...
     void setupWebSocketCallbacks() {
        ...
        ws_client->setBinaryCallback([this](const char* data, size_t size) -> bool {
         ...
         ws_client->setStateCallback([this](websocket_ns::ConnectionState old_state,
            ...
            } else if (new_state == websocket_ns::ConnectionState::CONNECTING) {
                // 连接中状态，可能是重连
                LOG_INFO("ChatbotV2", "WebSocket reconnecting...");
                setState(ChatbotState::CONNECTING);
            }
         });
 ...
 // ========================================================================
    ...
    void handleReconnect() {
        ...
        uint64_t last_check = last_reconnect_check.load(std::memory_order_acquire);
        if (now - last_check < RECONNECT_CHECK_INTERVAL_US) {
            LOG_DEBUG("ChatbotV2", "Reconnect check too frequent, skipping (last: %llu, now: %llu, diff: %llu us)", 
                     last_check, now, now - last_check);
            return; // 太频繁，跳过这次检查
        }
        
        // 更新检查时间
        last_reconnect_check.store(now, std::memory_order_release);
        
        // 检查是否需要重连
        if (ws_client->shouldReconnect()) {
            LOG_INFO("ChatbotV2", "Processing WebSocket reconnect...");
            
            // 处理重连
            ws_client->processReconnect();
            
            // 等待重连完成
            int wait_count = 0;
            while (wait_count < 50) {  // 最多等待5秒
                if (ws_client->isHandshaked()) {
                    LOG_INFO("ChatbotV2", "✓ WebSocket reconnected successfully!");
                    setState(ChatbotState::CONNECTED);
                    
                    // 重连成功后，重新发送Hello消息
                    if (protocol_handler) {
                        std::string hello_msg = protocol_handler->generateHelloMessage(
                            16000,  // 16kHz
                            1,      // Mono
                            20      // 20ms frame
                        );
                        ws_client->setHelloMessage(hello_msg);
                        LOG_INFO("ChatbotV2", "Hello message updated for reconnection");
                    }
                    return;
                }
            ...
            LOG_WARN("ChatbotV2", "WebSocket reconnect timeout");
        } else {
            LOG_DEBUG("ChatbotV2", "WebSocket does not need reconnection");
        }
    }
...
void ChatbotSystemV2::triggerReconnect() {
    LOG_INFO("ChatbotV2", "Manual WebSocket reconnect trigger");
    
    if (pImpl_->ws_client) {
        pImpl_->handleReconnect();
    } else {
        LOG_WARN("ChatbotV2", "WebSocket client not available");
    }
}
