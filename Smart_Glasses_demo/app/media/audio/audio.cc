 ...
 */
 ...
 void AudioMemoryPool::logStats() const {
     ...
     if (fixed_rate < 70.0 && total > 100) {
         LOG_WARN("AudioBuffer", "固定池命中率偏低 (%.2f%%)，建议增加池大小或检查内存泄漏", fixed_rate);
     }
 }
 ...
 public:
     ...
     void setControlState(AudioControlState new_state) {
         ...
         if (old_state != new_state) {
             LOG_DEBUG("AudioSystem", "控制状态: %d → %d",
                      static_cast<int>(old_state), static_cast<int>(new_state));
 ...
 AudioError AudioSystem::initialize(std::shared_ptr<sync_context_t> sync_ctx) {
     ...
     if (pImpl_->config.enable_denoise || pImpl_->config.enable_agc) {
         ...
         if (!speex) {
             ...
             LOG_INFO("AudioSystem", "  3A: 降噪=%s, AGC=%s",
                      denoise ? "开启" : "关闭", agc ? "开启" : "关闭");
         }
     }
 ...
 AudioError AudioSystem::setFunctionState(AudioFunctionState state) {
     ...
     if (old_state != state) {
         LOG_DEBUG("AudioSystem", "功能状态: %d → %d",
 ...
 int AudioSystem::Impl::recordCallback(const void* inputBuffer, void* outputBuffer,
                                         unsigned long framesPerBuffer,
                                         const PaStreamCallbackTimeInfo* timeInfo,
                                         PaStreamCallbackFlags statusFlags,
                                         void* userData) {
     ...
     if (main_state_ok && streaming_ok && encoder_ok && resampler_ok) {
        ...
        //          LOG_DEBUG("AudioCallback", "AI音频处理中... (帧 %d)", ai_frame_count.load());
 ...
 int AudioSystem::Impl::playbackCallback(const void* inputBuffer, void* outputBuffer,
                                           unsigned long framesPerBuffer,
                                           const PaStreamCallbackTimeInfo* timeInfo,
                                           PaStreamCallbackFlags statusFlags,
                                           void* userData) {
 ...
 AudioError AudioSystem::startRecord() {
     ...
     LOG_INFO("AudioSystem", "录音已启动（设备: %d, %dHz, %d声道）",
 ...
 AudioError AudioSystem::startPlayback() {
     ...
     LOG_INFO("AudioSystem", "播放已启动（设备: %d）", outputParams.device);
 ...
 AudioFramePtr AudioSystem::decodeOpus(const uint8_t* opus_data, size_t opus_size) {
     ...
     if (decoded_samples != frame_size) {
         LOG_WARN("AudioSystem", "解码样本数不匹配: 获得 %d，期望 %d", 
     ...
     if (count < 10) {
         ...
         LOG_INFO("AudioSystem", "  PCM #%d: 样本数=%d, 范围=[%d, %d]", 
 ...
 size_t AudioSystem::encodeOpusFrames(const int16_t* pcm_data, size_t pcm_size, 
     ...
     LOG_DEBUG("AudioSystem", "编码了 %zu 个样本为 %zu 帧", total_samples, encoded_count);
 ...
 AudioError AudioSystem::startAIStream() {
     ...
     LOG_DEBUG("AudioSystem", "    - 主状态: %d", static_cast<int>(pImpl_->main_state.load()));
     LOG_DEBUG("AudioSystem", "    - is_ai_streaming: %d", pImpl_->is_ai_streaming.load());
