...
JpegEncoder::~JpegEncoder() { deinit(); }
Error JpegEncoder::init(const JpegCfg& cfg, FramePool* pool) { return impl_->init(cfg, pool); }
void JpegEncoder::deinit() { impl_->deinit(); }
bool JpegEncoder::is_init() const { return impl_->init_; }
Error JpegEncoder::start() { return impl_->start(); }
Error JpegEncoder::stop()
{
    impl_->stop();
    return Error::OK;
}
bool JpegEncoder::is_running() const { return impl_->running_; }
...
Error JpegEncoder::save(const std::string& path, PhotoCb cb) { return impl_->save(path, cb); }
const JpegCfg& JpegEncoder::cfg() const { return impl_->cfg_; }
