    ...
    namespace tool
    {
        namespace log
        {
            ...
            struct LogConfig
            {
                ...
                LogConfig()
                    : enable_console(true), enable_file(true), enable_color(true),
            ...
            struct LogMessage
            {
                ...
                LogMessage()
                    : level(LogLevel::INFO), timestamp(0), thread_id(std::this_thread::get_id())
                {
                }
            };
...
#define LOG_DEBUG(tag, format, ...)                                                                \
    app::tool::log::Logger::getInstance().log(app::tool::log::LogLevel::DEBUG, tag, format,        \
                                              ##__VA_ARGS__)

#define LOG_INFO(tag, format, ...)                                                                 \
    app::tool::log::Logger::getInstance().log(app::tool::log::LogLevel::INFO, tag, format,         \
                                              ##__VA_ARGS__)

#define LOG_WARN(tag, format, ...)                                                                 \
    app::tool::log::Logger::getInstance().log(app::tool::log::LogLevel::WARN, tag, format,         \
                                              ##__VA_ARGS__)

#define LOG_ERROR(tag, format, ...)                                                                \
    app::tool::log::Logger::getInstance().log(app::tool::log::LogLevel::ERROR, tag, format,        \
                                              ##__VA_ARGS__)
