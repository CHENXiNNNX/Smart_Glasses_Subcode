    ...
    namespace protocol
    {
        namespace websocket
        {
            ...
            using ConnectionStateCallback =
                std::function<void(ConnectionState old_state, ConnectionState new_state)>;
            using WebSocketErrorCallback =
                std::function<void(WebSocketError error, const std::string& message)>;

            class WebSocketClient
            {
            public:
                explicit WebSocketClient(WebSocketConfig config = WebSocketConfig());
                ~WebSocketClient();
            ...
            inline std::unique_ptr<WebSocketClient>
            createXiaozhiClient(const std::string& device_id, const std::string& client_id,
