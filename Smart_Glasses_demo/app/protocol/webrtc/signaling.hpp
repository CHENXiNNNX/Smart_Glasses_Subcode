 ...
 * @brief 错误回调
 */
using SignalingErrorCallback = std::function<void(SignalingError error, const std::string& message)>;
...
public:
     ...
     * @brief 获取连接状态
     */
    SignalingStatus getStatus() const { return status_.load(std::memory_order_acquire); }
    
    /**
     * @brief 获取设备ID
     */
    const std::string& getDeviceId() const { return config_.deviceId; }
...
private:
     ...
     */
    void handleWebSocketState(websocket::ConnectionState oldState, websocket::ConnectionState newState);
    
    /**
     * @brief 处理WebSocket错误
     */
    void handleWebSocketError(websocket::WebSocketError error, const std::string& message);
     ...
     * @brief 设置状态
     */
    void setStatus(SignalingStatus newStatus);
     ...
     * @brief 触发WebRTC就绪回调
     */
    void invokeWebRTCReadyCallback(const std::string& role, const std::string& peerDeviceId);
