...
namespace
{
    ...
    void printModelInfo(const RKNNModel& model, const std::string& model_name)
    {
        ...
        LOG_INFO(LOG_TAG, "模型尺寸: %dx%dx%d", model.getModelWidth(), 
                 model.getModelHeight(), model.getModelChannel());
        ...
        for (uint32_t i = 0; i < model.getInputNum(); i++)
        {
            ...
            if (attr)
            {
                LOG_INFO(LOG_TAG, "输入[%d]: %s, 尺寸=[%d,%d,%d,%d], 大小=%d 字节",
                         i, attr->name, attr->dims[0], attr->dims[1], 
                         attr->dims[2], attr->dims[3], attr->size_with_stride);
            }
        }
        ...
        for (uint32_t i = 0; i < model.getOutputNum(); i++)
        {
            ...
            if (attr)
            {
                ...
                if (has_quant)
                {
                    LOG_INFO(LOG_TAG, "输出[%d]: %s, 尺寸=[%d,%d,%d,%d], 大小=%d 字节, 量化参数: zp=%d, scale=%f",
                             i, attr->name, attr->dims[0], attr->dims[1], 
                             attr->dims[2], attr->dims[3], attr->size_with_stride, zp, scale);
                }
                else
                {
                    LOG_INFO(LOG_TAG, "输出[%d]: %s, 尺寸=[%d,%d,%d,%d], 大小=%d 字节",
                             i, attr->name, attr->dims[0], attr->dims[1], 
                             attr->dims[2], attr->dims[3], attr->size_with_stride);
                }
            }
