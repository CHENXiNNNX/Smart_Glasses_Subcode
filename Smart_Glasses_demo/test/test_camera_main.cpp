...
int main()
{
    ...
    std::vector<QualityTest> quality_tests = {
        {100, "质量100 - 最高质量"}, {95, "质量95 - 极高质量"}, {80, "质量80 - 高质量"},
        {50, "质量50 - 中等质量"},   {30, "质量30 - 低质量"},   {1, "质量1 - 最低质量"}};
    ...
    std::vector<BitrateTest> bitrate_tests = {
        {2048, "test_2mbps_5s.h264"}, {4096, "test_4mbps_5s.h264"}, {8192, "test_8mbps_5s.h264"}};
    ...
    LOG_INFO(LOG_TAG, "  固定池命中:    %zu (%.2f%%)", stats.mem_stats.fixed_pool_hits.load(),
             stats.mem_stats.total_allocations.load() > 0
                 ? (double)stats.mem_stats.fixed_pool_hits.load() * 100.0 /
                       stats.mem_stats.total_allocations.load()
                 : 0.0);
    LOG_INFO(LOG_TAG, "  动态池命中:    %zu (%.2f%%)", stats.mem_stats.dynamic_pool_hits.load(),
             stats.mem_stats.total_allocations.load() > 0
                 ? (double)stats.mem_stats.dynamic_pool_hits.load() * 100.0 /
                       stats.mem_stats.total_allocations.load()
                 : 0.0);
    LOG_INFO(LOG_TAG, "  DMA池命中:     %zu (%.2f%%)", stats.mem_stats.dma_pool_hits.load(),
             stats.mem_stats.total_allocations.load() > 0
                 ? (double)stats.mem_stats.dma_pool_hits.load() * 100.0 /
                       stats.mem_stats.total_allocations.load()
                 : 0.0);
