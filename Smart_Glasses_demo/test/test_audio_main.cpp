...
namespace
{
    ...
    static void printStats(const AudioDrv& drv, int sec, uint32_t drops, uint32_t vad)
    {
        auto s = drv.stats();
        printf("\r[%3ds] 采集:%4u 编码:%4u 解码:%4u 播放:%4u 丢:%3u VAD:%4u 内存:%uKB  ", sec,
               s.capture_frames, s.encode_cnt, s.decode_cnt, s.playback_frames, drops, vad,
    ...
    int main()
    {
        ...
        drv.set_capture_cb(
            [&](const FramePtr& pcm)
            {
                ...
                long long      e = 0;
                for (uint32_t i = 0; i < pcm->samples; ++i)
                    e += static_cast<long long>(s[i]) * s[i];
                if (std::sqrt(static_cast<double>(e) / pcm->samples) > 500.0)
                    vad_cnt.fetch_add(1, std::memory_order_relaxed);
            });
        ...
        while (g_running.load(std::memory_order_relaxed))
        {
            ...
            printStats(drv, ++elapsed, drops.load(std::memory_order_relaxed),
                       vad_cnt.load(std::memory_order_relaxed));
        }
        ...
        auto s = drv.stats();
        LOG_INFO(TAG, "统计 %ds 采集%u 编码%u 解码%u 播放%u 丢%u 内存%uKB", elapsed,
                 s.capture_frames, s.encode_cnt, s.decode_cnt, s.playback_frames, drops.load(),
                 static_cast<unsigned>(s.mem_used / 1024));
        if (s.capture_frames > 0)
        {
            LOG_INFO(TAG, "丢帧率%.1f%% 人声%.1f%%", drops.load() * 100.0 / s.capture_frames,
                     vad_cnt.load() * 100.0 / s.capture_frames);
        }
