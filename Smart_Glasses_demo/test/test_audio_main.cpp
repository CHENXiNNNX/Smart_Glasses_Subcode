 ...
 * @return 音频数据（int16格式）
 */
std::vector<int16_t> generateSineWave(double frequency, double amplitude, int sample_rate,
                                      int duration_ms)
{
 ...
 */
AudioFramePtr createSineWaveFrame(AudioMemoryPool& pool, double frequency, double amplitude,
...
void onRecordCallback(AudioFramePtr frame)
{
    ...
    {
        LOG_DEBUG(LOG_TAG, "已录制帧数: %d, 队列大小: %zu", g_callback_count.load(),
                  g_recorded_frames.size());
    }
}
...
bool testMemoryAllocation()
{
    ...
    LOG_INFO(LOG_TAG, "  固定池命中: %llu (%.2f%%)", stats.fixed_pool_hits.load(),
             stats.getFixedPoolHitRate());
...
bool testOpusCodecAndPlayback()
{
    ...
    if (recorded_frames.empty())
    {
        ...
        for (int i = 0; i < frames_needed; i++)
        {
            // 生成20ms的混合正弦波
            auto samples =
                generateSineWave(440.0 + i * 2, 0.3, config.sample_rate, config.frame_duration_ms);
    ...
    {
        auto opus_frame = audio_system.encodeOpus(pcm_frame->getData<int16_t>(), pcm_frame->size);
...
bool test3AAlgorithms()
{
    ...
    LOG_INFO(LOG_TAG, "  AGC: %s (目标电平: %.1f)", config.enable_agc ? "开启" : "关闭",
             config.agc_level);
