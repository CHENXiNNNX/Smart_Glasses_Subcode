...
public:
    SileroVad(const std::string& model_path, float threshold)
        : threshold_(threshold),
          env_(ORT_LOGGING_LEVEL_WARNING, "test-silero-vad"),
          session_options_(),
          allocator_()
    {
    ...
    bool infer(const int16_t* pcm_48k, size_t samples_48k, float& prob, double& infer_ms)
    {
        ...
        Ort::Value h_tensor =
            Ort::Value::CreateTensor<float>(memory_info, h_state_.data(), h_state_.size(), HIDDEN_DIMS, 3);
        Ort::Value c_tensor =
            Ort::Value::CreateTensor<float>(memory_info, c_state_.data(), c_state_.size(), HIDDEN_DIMS, 3);
...
void printUsage(const char* prog)
{
    ...
    std::cout << "示例: " << prog << " -m ./models/silero_vad.onnx -t 0.60\n";
}
...
int main(int argc, char* argv[])
{
    ...
    drv.set_capture_cb(
        [&](const FramePtr& pcm)
        {
            ...
            while (!stats.total_infer_ms.compare_exchange_weak(
                old_total, old_total + infer_ms, std::memory_order_relaxed))
            {
            }
    ...
    while (g_running.load(std::memory_order_relaxed))
    {
        ...
        double   sec_ratio =
            sec_infer > 0 ? (static_cast<double>(sec_speech) / static_cast<double>(sec_infer)) : 0.0;
