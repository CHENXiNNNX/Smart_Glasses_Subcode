/**
 * @file test_ai_chatbot.cpp
 * @brief xiaozhi AI聊天机器人完整测试
 * @details 测试AIManager的完整功能：
 *          - WebSocket连接
 *          - 音频推流
 *          - STT/LLM/TTS处理
 *          - MCP设备注册
  ...
  int main() {
     ...
     std::cout << "\n[Test] Step 4: Setup callbacks..." << std::endl;
      ...
      ai_manager.onStateChanged([](AIManagerState state) {
          std::cout << "[Callback] State changed to: " << static_cast<int>(state) << std::endl;
      });
     ...
     std::cout << "\n[Test] Step 8: Start AI service..." << std::endl;
      ...
      std::cout << "========================================" << std::endl;
      std::cout << "\n操作提示:" << std::endl;
      std::cout << "  1. 按 's' 开始监听" << std::endl;
      std::cout << "  2. 按 'x' 停止监听" << std::endl;
      std::cout << "  3. 按 'q' 退出程序" << std::endl;
     std::cout << "  4. 按 'i' 显示状态信息" << std::endl;
     std::cout << "\n等待命令..." << std::endl;
     ...
     bool is_listening = false;
      ...
      while (g_running) {
          ...
          if (ch == 's' || ch == 'S') {
              if (!is_listening) {
                  std::cout << "\n[Test] Starting listening..." << std::endl;
                  if (ai_manager.startListening("auto")) {
                      is_listening = true;
                      std::cout << "[Test] ✓ 请开始说话..." << std::endl;
                  }
              } else {
                  std::cout << "[Test] Already listening!" << std::endl;
              }
          } 
          else if (ch == 'x' || ch == 'X') {
              if (is_listening) {
                  std::cout << "\n[Test] Stopping listening..." << std::endl;
                  if (ai_manager.stopListening()) {
                      is_listening = false;
                      std::cout << "[Test] ✓ Listening stopped" << std::endl;
                  }
              } else {
                  std::cout << "[Test] Not listening!" << std::endl;
              }
          }
          else if (ch == 'i' || ch == 'I') {
              std::cout << "\n========================================" << std::endl;
