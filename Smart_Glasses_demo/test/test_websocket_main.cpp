...
void testFullConnection()
{
    ...
    if (!result.isActivated())
    {
        ...
        // 设置回调
        activation.setStatusCallback(
            [](ActivationStatus status, const ActivationResult& /*result*/)
            {
        ...
        activation.setProgressCallback(
            [](int elapsed_sec, int total_sec) {
                std::cout << "[进度] 已等待: " << elapsed_sec << "/" << total_sec << " 秒"
                          << std::endl;
            });
    ...
    // 设置协议回调
    protocol_handler.setHelloCallback(
        [](const HelloMessage& msg)
        {
            ...
            std::cout << "  音频参数: " << msg.audio_params.sample_rate << "Hz, "
                      << msg.audio_params.channels << "声道, " << msg.audio_params.frame_duration
                      << "ms" << std::endl;
        });

    protocol_handler.setSTTCallback(
        [](const STTMessage& msg)
        {
            std::cout << "\n[协议] ← STT: \"" << msg.text
                      << "\" (完成: " << (msg.is_final ? "是" : "否") << ")" << std::endl;
        });

    protocol_handler.setLLMCallback(
    ...
    protocol_handler.setTTSCallback(
    ...
    ws_client.setTextCallback(
        [&protocol_handler](const char* data, size_t size) -> bool
        {
    ...
    std::string listen_msg =
        protocol_handler.generateListenMessage(ListenState::START, ListenMode::AUTO);
